...
import com.respireplus.respire.Math.Fft;
import com.respireplus.respire.Math.Fft2;
    ...
    private  PreviewCallback previewCallback = new PreviewCallback() {
        ...
        public void onPreviewFrame(byte[] data, Camera cam) {
            ...
            if (totalTimeInSecs >= 30) { //when 30 seconds of measuring passes do the following " we chose 30 seconds to take half sample since 60 seconds is normally a full sample of the heart beat
                ...
                if ((bufferAvgB < 45 || bufferAvgB > 200) && (bufferAvgBr < 10 || bufferAvgBr > 24)) { //if the heart beat wasn't reasonable after all reset the progresspag and restart measuring
                    inc=0;
                    ProgP=inc;
                    ProgHeart.setProgress(ProgP);
                    mainToast = Toast.makeText(getApplicationContext(), "Critical Readings Observed / Measurement Failed", Toast.LENGTH_SHORT);
                    mainToast.show();
                    startTime = System.currentTimeMillis();
                    counter=0;
                    processing.set(false);
            ...
            if((Beats != 0)&&(Breath != 0)){ //if beasts were reasonable stop the loop and send HR with the username to results activity and finish this activity

                Intent returnIntent = new Intent();
                returnIntent.putExtra("bpm",String.valueOf(Beats));
